...
func _ready() -> void:
	MultiplayerManagement.change_map("res://multiplayer/multiplayer_lobby.tscn")
...
func on_lobby_created(result: int, lobby_id: int):
	if result == Steam.Result.RESULT_OK:
		...
		add_player()
		MultiplayerManagement.join_code = lobby_id
		print(lobby_id)
...
func on_lobby_joined(lobby_id: int, _permissions: int, _locked: bool, _response: int):
	if !is_joining:
		return
